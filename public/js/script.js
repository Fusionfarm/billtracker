...
$('#contents_header_categories').live('click', function (e) {
	...
	$('#bill_content_init').html("<div class='loading_content_init'><h4 class='headers'>Loading...</h3></div><div id='bills_init'></div>");
	$('#bill_content').hide();
...
$('.actions_buttons').on('click', function (e) {
	e.preventDefault();
...
$('.votes_buttons').on('click', function (e) {
	e.preventDefault();
...
function loadCategories() {
	...
	$('.circles').each($).wait(200, function(){
		...
		var circle = paper.circle(62.5, 62.5, 55);
		var img = paper.image(img_src, 35, 35, 50, 50);
		var text = paper.text(circle.attrs.cx, 140, circle_id.capitalize())
		...
		layer_group.click(function(e) {
			...
			topic = $(e.target).parent().parent().attr('id');
...
function initialLoadJSON(topic) {
	...
	var topic = topic;
	var topic_format = topic.capitalize() + "";
